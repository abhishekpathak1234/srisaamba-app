-- ════════════════════════════════════════════════════════════════
-- Phase 2 · Dynamic Dashboard Foundation
-- AutoClient (Sri Saamba AI)
--
--   Tables:  call_logs, customers, appointments, dealership_metrics,
--            tasks
--   RLS:     dealer_id isolation on every table via
--            current_dealership_id() (from the Phase 1 migration)
--   RPC:     seed_demo_data() — bootstraps a dealership if the user
--            has none, then loads realistic demo data once
--
-- Requires the Phase 1 migration to be applied first.
...
create table public.customers (
  ...
  status      text not null default 'new'
                check (status in ('new','engaged','booked','sold','lost')),
...
create table public.call_logs (
  ...
  call_type        text not null default 'inbound'
                     check (call_type in ('inbound','outbound','after_hours')),
  call_status      text not null default 'answered'
                     check (call_status in ('answered','missed','recovered','escalated')),
...
create table public.appointments (
  ...
  appointment_type text not null default 'test_drive'
                     check (appointment_type in ('test_drive','service','trade_in_inspection')),
  vehicle          text,  -- shown in the bookings tables (e.g. "2020 Toyota Camry · $17,500")
...
create table public.dealership_metrics (
  dealer_id              uuid primary key references public.dealerships(id) on delete cascade,
...
create table public.tasks (
  ...
  task_type   text not null
                check (task_type in ('hot_lead','escalation','follow_up','confirmation')),
  priority    text not null default 'normal'
                check (priority in ('urgent','high','normal','low')),
  status      text not null default 'open'
                check (status in ('open','done','dismissed')),
  title       text not null,
  description text,  -- shown under the title in the Action Center cards
  assigned_to uuid references auth.users(id) on delete set null,
  due_at      timestamptz,
  created_at  timestamptz not null default now()
);

create index customers_dealer_idx     on public.customers (dealer_id, created_at desc);
create index call_logs_dealer_idx     on public.call_logs (dealer_id, created_at desc);
create index appointments_dealer_idx  on public.appointments (dealer_id, scheduled_at);
create index tasks_dealer_idx         on public.tasks (dealer_id, status, created_at desc);

-- keep dealership_metrics.updated_at fresh (trigger fn from Phase 1)
create trigger dealership_metrics_set_updated_at
  before update on public.dealership_metrics
  for each row execute function public.set_updated_at();

-- ── 2 · Row Level Security ──────────────────────────────────────
-- One rule everywhere: you can only touch rows whose dealer_id is
-- your own dealership. current_dealership_id() (Phase 1, SECURITY
-- DEFINER) resolves that from auth.uid().

alter table public.customers          enable row level security;
alter table public.call_logs          enable row level security;
alter table public.appointments       enable row level security;
alter table public.dealership_metrics enable row level security;
alter table public.tasks              enable row level security;

create policy "members_all_customers"
  on public.customers for all to authenticated
  using (dealer_id = public.current_dealership_id())
  with check (dealer_id = public.current_dealership_id());

create policy "members_all_call_logs"
  on public.call_logs for all to authenticated
  using (dealer_id = public.current_dealership_id())
  with check (dealer_id = public.current_dealership_id());

create policy "members_all_appointments"
  on public.appointments for all to authenticated
  using (dealer_id = public.current_dealership_id())
  with check (dealer_id = public.current_dealership_id());

create policy "members_all_metrics"
  on public.dealership_metrics for all to authenticated
  using (dealer_id = public.current_dealership_id())
  with check (dealer_id = public.current_dealership_id());

create policy "members_all_tasks"
  on public.tasks for all to authenticated
  using (dealer_id = public.current_dealership_id())
  with check (dealer_id = public.current_dealership_id());

-- ── 3 · Demo data seeder ────────────────────────────────────────
-- Called by the dashboard on first load. Idempotent:
--   · user has no dealership  → creates one + owner membership
--     (replaces the removed onboarding step)
--   · dealership has no data  → loads realistic demo rows
--   · already seeded          → returns immediately
